...
description: >-
  ...
  occur. Use when debugging live issues, Sentry alerts, or Sidekiq dead sets — not
...
- **Sidekiq** retries exhausted, dead jobs, or cron not firing.
...
2. **Dashboards** — Sidekiq Web at **`/sidekiq`** (admin): queues, retries, cron. Redis connectivity (**`REDIS_URL`** name in README / **`.env.example`** only).
...
5. **Mitigate** — Scale workers, disable a noisy job, or rollback deploy if needed; document the decision.
...
- Local reproduction: **`docker-and-local-dev`** or **`bin/dev`** with sanitized data.
