...
/**
 * Insight workspace: pick one insight, then use all four discovery tools (interview guide,
 * survey, assumption map, competitor scan) on a single page. The Discover *hub* at
 * `/app/discover` explains how this route fits next to the board and the insights list.
 */
export default async function DiscoverWorkspacePage({
  searchParams,
}: {
  ...
  if (!(await userHasProjectAccess(userId, projectId))) {
    return <ProjectAccessDenied pageTitle="Insight workspace" />;
  }
  ...
  return (
    <PageShell width="full">
      <PageHeader
        title="Insight workspace"
        description={project ? `${project.name} — validate one insight with the four tools` : "Validate one insight with the four tools"}
        back={{ href: "/app/discover", label: "Discover" }}
        actions={
      ...
      {noteParam === "empty_findings" && hasValidInsight && selectedInsight ? (
        <div className="alert alert-info d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-2 mb-4">
          ...
          <Link className="btn btn-sm btn-outline-info text-nowrap" href={`/app/discover/workspace?insight=${insightId}`}>
            Dismiss
          </Link>
        </div>
      ) : null}
