...
export async function runJob(job: Job): Promise<void> {
  ...
  switch (job.name) {
    ...
    case "SendDailyPulseJob": {
      ...
      for (const projectId of projectIds) {
        ...
        if (recipientEmails.length > 0) {
          const subject = `xenoform.ai pulse - ${periodStart.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`;
    ...
    case "ResendPulseReportJob": {
      ...
      const subject = `xenoform.ai pulse - ${report.periodStart.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })} (resent)`;
    ...
    case "SendPasswordResetJob": {
      ...
      await sendEmail({ to: email, subject: "Reset your xenoform.ai password", html, text });
